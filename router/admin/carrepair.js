...
carRepairAdminRoute.get("/", async (req, res) => {
  try {
    ...
    const listCarRepair = await CarRepair.find({ $and: [query, listKeyword] })
      ...
      .sort(`${sortBy}`)
    if (listCarRepair)
      return sendSuccess(res, "Get Carrepair successful", {
        length,
        listCarRepair,
      })
 ...
 * @access private
 */
carRepairAdminRoute.get("/:id", async (req, res) => {
  try {
    const { id } = req.params
    const carrepairs = await CarRepair.findById(id);
    if (carrepairs)
      return sendSuccess(res, "Get carrepair successful", carrepairs)
...
carRepairAdminRoute.post("/create", async (req, res) => {
  ...
  try {
    const { car, repairCar_type, device, price, note, bill } = req.body
    const isExitCar = await Car.exists({ _id: car })

    if (!isExitCar) return sendError(res, "This Car is not existed")
    else
      await CarRepair.create({
        car,
        repairCar_type,
        device,
        price,
...
carRepairAdminRoute.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!isExit) return sendError(res, "Car not exitsts")
...
carRepairAdminRoute.put("/:id", async (req, res) => {
  try {
    ...
    if (!isExist) return sendError(res, "ID does not exits")
    const isExistCar = await Car.exists({ _id: car })
    if (!isExistCar) return sendError(res, "This car is not exited")
    await CarRepair.findByIdAndUpdate(id, {
      car,
      repairCar_type,
      device,
      price,
 ...
 * @access private
 */
carRepairAdminRoute.get("/car/:plate", async (req, res) => {
  const { plate } = req.params
  try {
    const car = await Car.findOne({ plate: plate })
    const carrepairs = await CarRepair.find({ car: car._id })
    if (carrepairs)
      return sendSuccess(res, "Get history repair successful", carrepairs)
    return sendError(res, "Not information found.")
  } catch (error) {
    console.log(error)
    return sendServerError(res)
