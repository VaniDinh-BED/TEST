...
distanceAdminRoute.post("/create/:serviceId", async (req, res) => {
  ...
  try {
    const service = await DeliveryService.findById(req.params.serviceId).populate("distances");
    if (service) {
      ...
      const distance = await Distance.create({ fromProvince, toProvince, zonecode, distance: dist, })
    ...
    return sendError(res, "Create distance Failed.")
  } catch (error) {
...
distanceAdminRoute.put("/:id", async (req, res) => {
  ...
  if (!id.match(/^[0-9a-fA-F]{24}$/)) return sendError(res, "Distance does not exist.")
  try {
    const distance = await Distance.findById(id);
    if (distance) {
      await Distance.findByIdAndUpdate(id, { fromProvince: fromProvince, toProvince: toProvince, zonecode: zonecode, distance: dist })
      return sendSuccess(res, "Update distance successfully.", { fromProvince: fromProvince, toProvince: toProvince, zonecode: zonecode, distance: dist })
    }
...
distanceAdminRoute.delete("/:id", async (req, res) => {
  const { id } = req.params
  try {
    if (!id) return sendError(res, "Distance does not exist.")
