...
staffAdminRoute.put('/:id', async (req, res) => {
    let id = req.params.id;
    const { name, staff_type, department, car_fleet, salary } = req.body;
    const isExist = await Staff.exists({ _id: id })
    if (!isExist) { return sendError(res, 'Staff does not exist.') }
    if (staff_type) {
    ...
    try {
        await Staff.findByIdAndUpdate(id, { name: name, staff_type, department: department, car_fleet: car_fleet, salary: salary })
