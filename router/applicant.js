...
applicantRoute.post(
  ...
  async (req, res) => {
    ...
    return sendError(res, " Create applicant failed.")
    const isexistcareer = await Career.exists({_id:careerId})
    if (!isexistcareer) return sendError(res, "Added applicant does not exist")
    const findCareer = await Career.findById(careerId);
    for(var i = 0; i < findCareer.applicants.length; i++) {
      const findApplicant = await Applicant.findOne(findCareer.applicants[i]);
      if (phoneNumber == findApplicant.phoneNumber || email == findApplicant.email) {
        return sendError(res, "Applicant be existed");
      }
    }
    try {
