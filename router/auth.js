...
authRoute.post('/register', async (req, res) => {
    ...
    let { name, email, password, phone, address, description, customer_type, taxcode, bank_name, bank_account_number, verify_op } = req.body
    ...
    try {
        ...
        req.session.register = JSON.stringify({
            name, email, password, phone, address, description, customer_type, taxcode, bank_name, bank_account_number, verify_op, otp
        })
        return sendSuccess(res, 'send otp code successfully.', otp)
    } catch (error) {
        console.log(error)
...
authRoute.post('/verify-otp', async (req, res) => {
    ...
    try {
        ...
        const { name, email, password, phone, address, description, customer_type, taxcode, bank_name, bank_account_number, otp } = JSON.parse(req.session.register)
 ...
 * @description customer register mobile
 * @access public
 */
authRoute.post('/register-mobile', async (req, res) => {
    const errors = customerRegisterValidate(req.body)
    if (errors)
        return sendError(res, errors)

    let { name, email, password, phone, address, name_product, quantity_order, verify_op } = req.body
    ...
    try {
        ...
        req.session.register = JSON.stringify({
            name, email, password, phone, address, name_product, quantity_order, verify_op, otp
        })
 ...
 * @description customer verify otp mobile
 * @access public
 */
authRoute.post('/mobile/verify-otp', async (req, res) => {
    const errors = userVerifyOTP(req.body)
    if (errors)
        return sendError(res, errors)
    ...
    try {
        if (!req.session.register)
            return sendError(res, 'Session error.', 404)
        const { name, email, password, phone, address, name_product, quantity_order, otp } = JSON.parse(req.session.register)
        if (req.body.otp !== otp.value || otp.expired < Date.now())
            return sendError(res, 'validate failed.')
        req.session.destroy()
        const newCustomer = await Customer.create({
