...
productRoute.post('/:orderId',
    ...
    async (req, res) => {
        try {
            ...
            if (order.timeline.slice(-1).status !== ORDER_STATUS.waiting_for_pickup) return sendError(res, "Orders can't add more products.")
            for (let i in products) {
                const product = products[i];
                const errors = addProductValidate(product)
                if (errors) return sendError(res, errors)
                 await Product.create({name: product.name, quantity: product.quantity, unit: product.unit, order, note: product.note})
               }
